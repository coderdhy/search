...
static void fts5HighlightPosAppend2(int *pRc, HighlightContext *p, const char *z, int n) {
	if (*pRc == SQLITE_OK) {
		if (n < 0) n = (int)strlen(z);
		p->zOut = sqlite3_mprintf("%z%.*s", p->zOut, n, z);
		if (p->zOut == 0) *pRc = SQLITE_NOMEM;
...
static void fts5HighlightPosAppendStart2(int *pRc, HighlightContext *p, int start) {
	std::string str = std::to_string(start);
	fts5HighlightPosAppend2(pRc, p, str.c_str(), -1);
	fts5HighlightPosAppend2(pRc, p, ",", -1);
}

static void fts5HighlightPosAppendEnd2(int *pRc, HighlightContext *p, int end) {
	std::string str = std::to_string(end);
	fts5HighlightPosAppend2(pRc, p, str.c_str(), -1);
	fts5HighlightPosAppend2(pRc, p, ";", -1);
}


int getUtfStart(const char *s, int cStr) {
	int count = 0;
	int strLength = strlen(s);
	for (int i = 0; i < strLength && i < cStr; i++) {
		count += (*s++ & 0xC0) != 0x80;
	}
	return count;
}
...
static int fts5HighlightCb2(void *pContext,     /* Pointer to HighlightContext object */
	int tflags,         /* Mask of FTS5_TOKEN_* flags */
	const char *pToken, /* Buffer containing token */
	int nToken,         /* Size of token in bytes */
	int iStartOff,      /* Start offset of token */
	...
	if (iPos == p->iter.iStart) {
		//    fts5HighlightAppend(&rc, p, &p->zIn[p->iOff], iStartOff - p->iOff);
		//    fts5HighlightAppend(&rc, p, p->zOpen, -1);
		int utf8Start = getUtfStart(p->zIn, iStartOff);
		fts5HighlightPosAppendStart2(&rc, p, utf8Start);
		p->iOff = iStartOff;
	}
	...
	if (iPos == p->iter.iEnd) {
		...
		//    fts5HighlightAppend(&rc, p, &p->zIn[p->iOff], iEndOff - p->iOff);
		//    fts5HighlightAppend(&rc, p, p->zClose, -1);
		int utf8End = getUtfStart(p->zIn, iEndOff);
		fts5HighlightPosAppendEnd2(&rc, p, utf8End);
...
static void fts5HighlightPosAppendStart(int *pRc, HighlightPosContext *p, int start) {
	std::string str = std::to_string(start);
	fts5HighlightPosAppend(pRc, p, str.c_str(), -1);
...
extern "C" void simple_highlight_pos2(const Fts5ExtensionApi *pApi, /* API offered by current FTS version */
	...
	if (ctx.zIn) {
		...
		if (rc == SQLITE_OK) {
			rc = pApi->xTokenize(pFts, ctx.zIn, ctx.nIn, (void *)&ctx, fts5HighlightCb2);
		}
